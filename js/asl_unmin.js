document.addEventListener('mousedown', function(e){
	...
	if(e.target && ( nn == 'select' || nn == 'input' || nn == 'textarea')){
		return;
	}
	
	e.preventDefault();
	var result = window.hasClick(e.target);
...
document.addEventListener('click', function(e){
	if(!e.custom){
		var nn = e.target.nodeName.toLowerCase();
		if( nn != 'select' && nn != 'input' && nn != 'textarea'){
			e.preventDefault();
		}
	}
}, false);

window.hasClick = function(elem){
	while(elem){
		if((elem.getAttribute && elem.getAttribute('onclick')) || elem.onclick){
			return true;
		}
...
var _paramsTranslator = {
	adapterpowerstateevent : 'adapterPowerStateEvent',
    adaptivescanning : 'adaptiveScanning',
    addedprofileevent : 'addedProfileEvent',
    addprofile : 'addProfile',
    aimmode : 'aimMode',
    aimtype : 'aimType',
    alldecoders : 'allDecoders',
    allprofilesevent : 'allProfilesEvent',
    audiosaveevent : 'audioSaveEvent',
    auspostal : 'ausPostal',
    autoenter : 'autoEnter',
    autotab : 'autoTab',
    aztec : 'aztec',
    bidirectionalredundancy : 'bidirectionalRedundancy',
    cancel : 'cancel',
    cancelloginresponseevent : 'cancelLoginResponseEvent',
    cancelnetworklogin : 'cancelNetworkLogin',
    canpostal : 'canPostal',
    chinese2of5 : 'chinese2of5',
    codabar : 'codabar',
    codabarclsiediting : 'codabarClsiEditing',
    codabarmaxlength : 'codabarMaxLength',
    codabarminlength : 'codabarMinLength',
    codabarnotisediting : 'codabarNotisEditing',
    codabarredundancy : 'codabarRedundancy',
    code11 : 'code11',
    ...
    code128 : 'code128',
    code128checkisbttable : 'code128checkIsBtTable',
    ...
    currentprofileevent : 'currentProfileEvent',
    d2of5 : 'd2of5',
	...
	enumscannerevent : 'enumScannerEvent',	
    ...
    disconnectbtondisable : 'disconnectBtOnDisable',
    dpmmode : 'dpmMode',
    duration : 'duration',
    ...
    enumerate : 'enumerate',
    focusmode : 'focusMode',
    getadapterpowerstate : 'getAdapterPowerState',
    ...
    importconfigevent : 'importConfigEvent',
    importconfigfromlocation : 'importConfigFromLocation',
    inverse1dmode : 'inverse1dMode',
    jappostal : 'japPostal',
    keyevent : 'keyEvent',
    klasseeins : 'klasseEins',
    korean3of5 : 'korean3of5',
    ...
    logincandidateprofilesevent : 'loginCandidateProfilesEvent',
    loginresponseevent : 'loginResponseEvent',
    logoutresponseevent : 'logoutResponseEvent',
    ...
    macromicropdfreportappendinfo : 'macroMicroPdfReportAppendInfo',
    macropdf : 'macroPdf',
    ...
    networklogincloseevent : 'networkLoginCloseEvent',
    networkloginevent : 'networkLoginEvent',
    ...
    poorquality1dmode : 'poorQuality1dMode',
    port : 'port',
    profileencryption : 'profileEncryption',
    profileessid : 'profileEssid',
    profileguid : 'profileGuid',
    profilename : 'profileName',
    profilepassphrase : 'profilePassphrase',
    profilesecuritymode : 'profileSecurityMode',
    qrcode : 'qrCode',
    ...
    signatureimagesize : 'signatureImageSize',
    signatureimagewidth : 'signatureImageWidth',
    source : 'source',
    start : 'start',
    stop : 'stop',
    timedaimduration : 'timedAimDuration',
    tlc39 : 'tlc39',
    top : 'top',
    transfer : 'transfer',
    ...
    upceanconvertrsstoupcean : 'upcEanConvertRssToUpcEan',
    ...
    viewfinderfeedback : 'viewfinderFeedback',
    viewfinderfeedbacktime : 'viewfinderFeedbackTime',
...
var signal = (function() {
    ...
    var obj = {
        ...
        getEventResponseProperties : function(evt) {
            switch(evt) {
                case 'signalEvent':
                    return ['signalStrength', 'essid', 'macAddress', 'adapterName', 'dhcpServer', 'dhcpStatic', 'gateway', 'ipAddress', 'rssi', 'subnetMask', 'wins'];
            }
        },
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('signal', emml);
    	}
    };
...
var battery = {
	getSmartBatteryStatus:function(){
		//not implemented
	},
...
var scanner = (function() {
    ...
    var obj = {
    	...
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('scanner', emml);
    	}
    };
...
var generic = (function() {
    ...
    var obj = {
		_execMethod:function(object, method){
            if(method != 'enabled' && object[method]) {
		...
		_parseSingleElement : function(object, content, property){
			if(content.indexOf(':') < 0){
				//this is a function call
				//var method = content.substr(0, 1).toLowerCase() + content.substr(1);
				var method = _paramsTranslator[content.toLowerCase()];
				this._execMethod(object, method);
			}
			else{
				//callback or a property
				if(property.indexOf('url') != -1) {
					//defining a callback
					var params = property.split(':url');
                    ...
                    if(!params[1].match(/javascript/i)) {
                        ...
                        params[1] = params[1].replace(/\(['"]{1}(.*)['"]{1}\)/, '$1');
						params[1] = params[1][params[1].length-1] == ';' ? params[1].slice(0, params[1].length-1) : params[1];
						params[1] = params[1][params[1].length-1] == ')' ? params[1].slice(0, params[1].length-1) : params[1];
						if(params[1].indexOf('http://') > -1){
							params[1] = params[1].slice(params[1].indexOf('http://'), params[1].length);
						}
						if(params[1].indexOf('file://') > -1){
							params[1] = params[1].slice(params[1].indexOf('file://'), params[1].length);
						}
						if(params[1].indexOf('ftp://') > -1){
							params[1] = params[1].slice(params[1].indexOf('ftp://'), params[1].length);
						}
                    } else {
					...
					object[_paramsTranslator[params[0].toLowerCase()]] = params[1];
				}
				else{
					//this case it is a property setter
        ...
        InvokeMETAFunction : function(module, content) {
			...
			if(module.indexOf('-') > 0){
				...
				module = modules[0];
				content = modules[1] + ':' + content; 	
			}
			
			var moduleLowerCase = module.toLowerCase();
			if(['scanner', 'audiocapture', 'filetransfer', 'push', 'signal', 'wlan', 'keycapture'].indexOf(moduleLowerCase) !== -1) {
                //var object = eval(module);
                var object = window[_moduleTranslator[moduleLowerCase]];
				if(content.indexOf(':') === -1) {
					this._parseSingleElement(object, content);
                } else {
                    ...
                    for(var i = 0, length = properties.length; i < length; i++) {
						try{
							if(properties[i].length > 0){
								this._parseSingleElement(object, properties[i], properties[i]);
							}
						}
						catch(e){}
                    }
                ...
                try {
					if(['battery'].indexOf(moduleLowerCase) !== -1) {
	                    try{
    ...
    Object.defineProperty(obj, 'OEMInfo', {
        get : function() {
             return _generic.OEMInfo; 
        },
        set : function(value){ 
...
var audioCapture = (function() {
    ...
    var obj = {
    	...
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('audioCapture', emml);
    	}
    };
...
var fileTransfer = (function() {
    ...
    var obj = {
    	...
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('fileTransfer', emml);
    	}
    };
...
var push = (function() {
    ...
    var obj = {
        start : function() {
            throw 'Error: push cannot be started within an application.';
            //_push.start();
        },
        stop : function() {
            throw 'Error: push cannot be stopped within an application.';
			//_push.stop();
        },
        detected : '',
        getEventResponseProperties : function(evt) {
    	...
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('push', emml);
    	}
    };
    ...
    Object.defineProperty(obj, 'port', {
        ...
        set : function(value) {
			throw 'Error: push port cannot be changed within an application.';
			//port = value;
            //_push.port = value;
        },
        enumerable : true,
    ...
    Object.defineProperty(obj, 'passKey', {
        ...
        set : function(value) {
            throw 'Error: push passkey cannot be changed within an application.';
//            passKey = value;
            //_push.passKey = value;
        },
        enumerable : true,
    ...
    Object.defineProperty(obj, 'path', {
        ...
        set : function(value) {
            alert('Error: push path cannot be changed within an application.');
            path = value;
            //_push.path = value;
        },
        enumerable : true,
...
var wlan = (function() {
    ...
    var obj = {
		...
		enableDot11d: function(){
			_wlan.enableDot11d();
		},
		disableDot11d: function(){
			_wlan.disableDot11d();
		},
		getDot11dMode: function(){
			_wlan.getDot11dMode();
		},
		getCountryCode: function(){
			_wlan.getCountryCode();
		},
		getCountryList: function(){
			_wlan.getCountryList();
		},
		setCountryCode: function(){
			_wlan.setCountryCode();
		},
		disableProfile: function(){
			_wlan.disableProfile();
		},
		enableProfile: function(){
			_wlan.enableProfile();
		},
		validateOptionsPassword: function(){
			_wlan.validateOptionsPassword();
		},
		getOptionsPasswordState: function(){
			_wlan.getOptionsPasswordState();
		},
		getRegulatorySettings: function(){
			_wlan.getRegulatorySettings();
		},
		dummyWLANAPI1: function(){
			_wlan.dummyWLANAPI1();
		},
		dummyWLANAPI2: function(){
			_wlan.dummyWLANAPI2();
		},
		dummyWLANAPI3: function(){
			_wlan.dummyWLANAPI3();
		},
		dummyWLANAPI4: function(){
			_wlan.dummyWLANAPI4();
		},
    	setEMML:function(emml){
    ...
    Object.defineProperty(obj, 'importConfigFromLocation', {
		...
		var exportType = '';
	Object.defineProperty(obj, 'exportType', {
		get : function(){ return exportType; },  
		set : function(value){ 
			exportType = value;
			_wlan.exportType = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var exportConfigToLocation = '';
	Object.defineProperty(obj, 'exportConfigToLocation', {
		get : function(){ return exportConfigToLocation; },  
		set : function(value){ 
			exportConfigToLocation = value;
			_wlan.exportConfigToLocation = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var configFilePassword = '';
	Object.defineProperty(obj, 'configFilePassword', {
		get : function(){ return configFilePassword; },  
		set : function(value){ 
			configFilePassword = value;
			_wlan.configFilePassword = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var profileGuids = '';
	Object.defineProperty(obj, 'profileGuids', {
		get : function(){ return profileGuids; },  
		set : function(value){ 
			profileGuids = value;
			_wlan.profileGuids = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var regulatory11dMode = '';
	Object.defineProperty(obj, 'regulatory11dMode', {
		get : function(){ return regulatory11dMode; },  
		set : function(value){ 
			regulatory11dMode = value;
			_wlan.regulatory11dMode = value;
		},  
		enumerable : true,  
		configurable : true
	});
	
	var regulatoryCountryCode = '';
	Object.defineProperty(obj, 'regulatoryCountryCode', {
		get : function(){ return regulatoryCountryCode; },  
		set : function(value){ 
			regulatoryCountryCode = value;
			_wlan.regulatoryCountryCode = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var optionsPassword = '';
	Object.defineProperty(obj, 'optionsPassword', {
		get : function(){ return optionsPassword; },  
		set : function(value){ 
			optionsPassword = value;
			_wlan.optionsPassword = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam1 = '';
	Object.defineProperty(obj, 'dummyWLANParam1', {
		get : function(){ return dummyWLANParam1; },  
		set : function(value){ 
			dummyWLANParam1 = value;
			_wlan.dummyWLANParam1 = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam2 = '';
	Object.defineProperty(obj, 'dummyWLANParam2', {
		get : function(){ return dummyWLANParam2; },  
		set : function(value){ 
			dummyWLANParam2 = value;
			_wlan.dummyWLANParam2 = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam3 = '';
	Object.defineProperty(obj, 'dummyWLANParam3', {
		get : function(){ return dummyWLANParam3; },  
		set : function(value){ 
			dummyWLANParam3 = value;
			_wlan.dummyWLANParam3 = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam4 = '';
	Object.defineProperty(obj, 'dummyWLANParam4', {
		get : function(){ return dummyWLANParam4; },  
		set : function(value){ 
			dummyWLANParam4 = value;
			_wlan.dummyWLANParam4 = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam5 = '';
	Object.defineProperty(obj, 'dummyWLANParam5', {
		get : function(){ return dummyWLANParam5; },  
		set : function(value){ 
			dummyWLANParam5 = value;
			_wlan.dummyWLANParam5 = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dummyWLANParam6 = '';
	Object.defineProperty(obj, 'dummyWLANParam6', {
		get : function(){ return dummyWLANParam6; },  
		set : function(value){ 
			dummyWLANParam6 = value;
			_wlan.dummyWLANParam6 = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var exportConfigEvent = '';
	Object.defineProperty(obj, 'exportConfigEvent', {
		get : function(){ return exportConfigEvent; },  
		set : function(value){ 
			exportConfigEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var dot11dModeEvent = '';
	Object.defineProperty(obj, 'dot11dModeEvent', {
		get : function(){ return dot11dModeEvent; },  
		set : function(value){ 
			dot11dModeEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var countryCodeEvent = '';
	Object.defineProperty(obj, 'countryCodeEvent', {
		get : function(){ return countryCodeEvent; },  
		set : function(value){ 
	...
	var countryListEvent = '';
	Object.defineProperty(obj, 'countryListEvent', {
		get : function(){ return countryListEvent; },  
		set : function(value){ 
			countryListEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var profileStateEvent = '';
	Object.defineProperty(obj, 'profileStateEvent', {
		get : function(){ return profileStateEvent; },  
		set : function(value){ 
			profileStateEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var validateOptionsPasswordEvent = '';
	Object.defineProperty(obj, 'validateOptionsPasswordEvent', {
		get : function(){ return validateOptionsPasswordEvent; },  
		set : function(value){ 
			validateOptionsPasswordEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});

	var optionsPasswordStateEvent = '';
	Object.defineProperty(obj, 'optionsPasswordStateEvent', {
		get : function(){ return optionsPasswordStateEvent; },  
		set : function(value){ 
			optionsPasswordStateEvent = value;
		},  
		enumerable : true,  
		configurable : true
	});
	...
	var regulatorySettingsEvent = '';
    Object.defineProperty(obj, 'regulatorySettingsEvent', {
        get : function(){ return regulatorySettingsEvent; },  
        set : function(value){ 
            regulatorySettingsEvent = value;
        },  
        enumerable : true,  
        configurable : true
    });
	...
	var dummyWLANEvent1 = '';
    Object.defineProperty(obj, 'dummyWLANEvent1', {
        get : function(){ return dummyWLANEvent1; },  
        set : function(value){ 
            dummyWLANEvent1 = value;
        },  
        enumerable : true,  
        configurable : true
    });
	...
	var dummyWLANEvent2 = '';
    Object.defineProperty(obj, 'dummyWLANEvent2', {
        get : function(){ return dummyWLANEvent2; },  
        set : function(value){ 
            dummyWLANEvent2 = value;
        },  
        enumerable : true,  
        configurable : true
    });
	...
	var dummyWLANEvent3 = '';
    Object.defineProperty(obj, 'dummyWLANEvent3', {
        get : function(){ return dummyWLANEvent3; },  
        set : function(value){ 
            dummyWLANEvent3 = value;
        },  
        enumerable : true,  
        configurable : true
    });
	...
	var dummyWLANEvent4 = '';
    Object.defineProperty(obj, 'dummyWLANEvent4', {
        get : function(){ return dummyWLANEvent4; },  
        set : function(value){ 
            dummyWLANEvent4 = value;
        },  
        enumerable : true,  
        configurable : true
    });
...
var keyCapture = (function() {
    ...
    var obj = {
    	...
    	setEMML:function(emml){
    		generic.InvokeMETAFunction('keyCapture', emml);
    	}
    };

    var keyEvent = '';
    Object.defineProperty(obj, 'keyEvent', {
        get : function() {
            return keyEvent;
        },
        set : function(value) {
...
/**
 * @description function to return unique ids in the page for the text input fields
 * @type Object
 */
 asl.prv.inputs._lastId = 0;
asl.prv.inputs.getId = function(){
	return "textInput-" + (asl.prv.inputs._lastId++);	
};
...
asl.prv.hiAccepted = function(params){
	...
	asl.prv.tegroup = params.tegroup;		
	...
	asl.prv.preventScanner = params.preventScanner;
...
asl.prv.npapi = function(params) {
    ...
    if(params.data.object == 'push') {
		if(object[params.data.method]) {
			...
			var url_regex = /^url\(["'](ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?["']\)\;*?$/;
			var is_url = object[params.data.method].match(url_regex);

			if(is_url && is_url[1] && is_url[3]){
				object[params.data.method] = is_url[1] + '://' + is_url[3];
			}

            if(object[params.data.method].indexOf('http://') != 0) {
				...
				var url_param = address.split('?');

                var props = object.getEventResponseProperties(params.data.method);
                var i = 0;

				var cb_props = [];
				for(var j in params.data.data){
					cb_props.push(params.data.data[j]);
				}

				var param_values = [];
				if(url_param.length == 2){
					var p = url_param[1].split('&');
					for(var i=0; i<p.length; i++){
						var param = p[i].split('=');
						var prop = param[0].substr(0, 1).toLowerCase() + param[0].substr(1);
						param_values.push(param[0] + '=' + (cb_props[i] ? cb_props[i] : 'undefined')); 
					}
				}
				...
				if(url_params.length == 0){
					for(var p in params.data.data){
						param_values.push(p + "=" +  params.data.data[p]);
					}	
				}
				
				address = url_param[0] + '?' + (param_values ? param_values.join('&') : "");
    ...
    //Process all NPAPI events events
		...
		if(object[params.data.method]) {
			if(object[params.data.method].indexOf('http://') != 0) {
				...
				if(url_param.length == 2){
					...
					for(var i=0; i<p.length; i++){
						var param = p[i].split('=');
						var prop = param[0].substr(0, 1).toLowerCase() + param[0].substr(1);
						param_values.push(param[0] + '=' + params.data.data[prop]); 
					}
				}
				...
				if(url_params.length == 0){
					for(var p in props){
						param_values.push(props[p] + "=" +  params.data.data[props[p]]);
					}	
				}
				address = url_param[0] + '?' + (param_values ? param_values.join('&') : "");
			...
			if(params.data.method == 'decodeEvent'){
				if(keyCapture['keyEvent']){
					asl.prv.npapi({data:{data:{keyValue:params.data.data["data"]}, method:'keyEvent', object : 'keyCapture'}});
				}
			}
		}
		
		if(params.data.method.toLowerCase() == 'keyevent'){
			asl.prv.resetTimer();
		}
    }
};
...
asl.prv.keyboard = function(params){
    ...
    asl.prv.resetTimer();
    if (params.id) {
		if(asl.prv.inputs[params.id] && asl.prv.inputs[params.id].callback){
...
asl.prv.trackDOMInputs = function(){
		...
		if(input_list[i].type != 'button' && input_list[i].type != 'submit' &&
           ...
           input_list[i].type != 'image' && input_list[i].type != 'hidden'){
			...
			if(!asl.prv.inputs[id]){
				asl.prv.inputs[id] = {};
				input_list[id].addEventListener('click', function(e){
					asl.keyboard(e.target, function(id, text){
						input_list[id].setAttribute("value",text);
						var value = text;

						if(input_list[id].type == 'currency') value = eval(text);						
                        
                        input_list[id].value = value;
	...
	var areas_list = document.getElementsByTagName('textarea');
	for(var i=0; i < areas_list.length; i++){
		var id = areas_list[i].id;
		if(!id || id.length == 0){
		    //@TODO where is the definition of .getId() !!!
			id = asl.prv.inputs.getId();

			areas_list[i].setAttribute('id', id);
			
			if(!asl.prv.inputs[id]){
				asl.prv.inputs[id] = {};
				areas_list[id].addEventListener('click', function(e){
					asl.keyboard(e.target, function(id, text){
						areas_list[id].innerHTML = text;
					});
				});
			}
		}
...
asl.prv.apps = function(params) {
	if(params.data.type == 'startedApp'){
		asl.events.fire(asl.events.types.appStarted, params.data);
	}
	else{
...
asl.minimize = function() {
	try{
		//generic.Log('Minimize asl: ' + asl.prv.id + '/' + asl.prv.loaded, 1);
	}
	catch(e){}
 ...
 */
asl.keyboard = function(type, callback, text, title, maxlength){
	var params = {};
	asl.kbdOn = true;
	...
	if(type instanceof HTMLInputElement || type instanceof HTMLTextAreaElement) id = type.id;
	else{
	 	id = asl.prv.id;
		asl.prv.inputs[id] = {};
	}
	...
	params.inputId = id;
	if(type instanceof HTMLTextAreaElement){
		params.type = "text";
	}
	else{
	...
	if(type instanceof HTMLInputElement){
		var val1 = type.getAttribute('value');
		var val2 = type.value;
	...
	if(type instanceof HTMLTextAreaElement){
		text = val = type.innerHTML;				
	}
	
	params.value = type instanceof HTMLInputElement ? val : text ? text : "";
...
asl.fn.store = {
	...
	save: function(type, string){
        ...
        var lgMsg = 'Size: ' + JSON.stringify(localStorage).length;
        
        //try{generic.Log(lgMsg, 1);}catch(e){}
	},
    /**
...
window.addEventListener("message", function(e){
	...
	if(e.data){
		...
		if(e.data.type == asl.msgs.types.evt){
			type += '/' + e.data.params.evt;
		}
	}

	try{
		generic.Log('MSG : ' + type + '|' + window.location, 1);
	}
	catch(r){}
	...
	if(!asl.prv.shell && e.data && e.data.type == asl.msgs.types.hi){
		...
		try{
			...
			try{
				//generic.Log('ERR: Shell address : ' + asl.prv.id + '|' + window.location + ':' + err, 1);
			}
			catch(r){}
		}
	}
...
asl.events.subscribe(asl.events.types.signal, function(data) {
	...
	asl.prv.npapi({data:{data: data.json, object:'signal', method:'signalEvent'}});
});

asl.events.subscribe(asl.events.types.loaded, function(data) {
	if(!asl.prv.preventScanner){
		try{
			scanner.disable();
		}
		catch(e){}
...
title.setBack = function(callback, img){
	...
	this.elements.backButtonWrap.style.display = "block";
};
...
options.pageDown = function(){
	...
	if(end - start < asl.prv.listItemsByPage){
...
options.show = function(){
	...
	this.elements.options_background.style.display = "block";
};
...
options.hide = function(){
	...
	if(options.background_timeout > 0){
        var cx = this;
        var to = setTimeout(function(){
            cx.elements.options_background.style.display = "none";          
            clearTimeout(to);
            to = null;
            delete to;
        }, options.background_timeout);
    }
    else{
        this.elements.options_background.style.display = "none";            
    }
};
...
options.create = function(){
    this.elements.options_background = document.createElement('div');
	this.elements.container = document.createElement('div');
	this.elements.titlepanel = document.createElement('div');
	...
	for (var i = 0; i < asl.prv.listItemsByPage; i++){
		...
		this.elements.items[i].addEventListener('click', function(){
            if(options.list.paging.info.options[this['data-id']].callback && options.list.paging.info.options[this['data-id']].callback.data){
                if(options.list.paging.info.options[this['data-id']].callback.data.indexOf('window.location') > -1 
                || options.list.paging.info.options[this['data-id']].callback.data.indexOf('document.location') > -1){
                    options.background_timeout = 3000;
                }
            }
...
options.init = function(){
	...
	this.elements.backButton.addEventListener('mousedown', function(){
		options.hide();
	}, false);

	this.elements.pagingTop.addEventListener('mousedown', function(){
		options.pageUp();
	},false);

	this.elements.pagingBottom.addEventListener('mousedown', function(){
		options.pageDown();
	},false);
	...
	document.body.appendChild(this.setStyle('options_background'));
